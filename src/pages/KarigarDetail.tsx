...
import { ArrowLeft, User, Phone, MapPin, Hammer, Wallet, Coins, Printer } from "lucide-react";
...
const KarigarDetail = () => {
  ...
  const karigar = karigars.find((k: any) => k.id === id);
  const allLedger = ledgerEntries.filter((e: any) => e.entry_type === "karigar" && e.reference_id === id);
    ...
    return (
      <AppLayout>
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <Button variant="outline" className="mt-4" onClick={() => navigate("/karigars")}>
            <ArrowLeft className="w-4 h-4 mr-2" /> Back to Karigars
          </Button>
        </div>
      </AppLayout>
    );
  ...
  return (
    <AppLayout>
      <div className="space-y-6 animate-fade-in">
        <PrintHeader title="Karigar Ledger" subtitle={karigar.name} />

        <div className="flex items-center justify-between print:hidden">
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon" onClick={() => navigate("/karigars")}>
              <ArrowLeft className="w-5 h-5" />
            </Button>
            <div>
              <h1 className="text-2xl font-bold">{karigar.name}</h1>
              <p className="text-sm text-muted-foreground">Karigar Details & Ledger</p>
            </div>
          </div>
          ...
          <CardContent className="p-5">
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5">
              <Avatar className="w-20 h-20 border-2 border-border">
                <AvatarFallback className="text-2xl bg-primary/10 text-primary">{karigar.name?.[0]?.toUpperCase()}</AvatarFallback>
              </Avatar>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 flex-1">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-primary/10 rounded-lg"><User className="w-4 h-4 text-primary" /></div>
                  <div><p className="text-xs text-muted-foreground">Name</p><p className="font-medium text-sm">{karigar.name}</p></div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-primary/10 rounded-lg"><Hammer className="w-4 h-4 text-primary" /></div>
                  <div><p className="text-xs text-muted-foreground">Specialty</p><p className="font-medium text-sm">{karigar.specialty || "-"}</p></div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-primary/10 rounded-lg"><Phone className="w-4 h-4 text-primary" /></div>
                  <div><p className="text-xs text-muted-foreground">Phone</p><p className="font-medium text-sm">{karigar.phone || "-"}</p></div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-primary/10 rounded-lg"><MapPin className="w-4 h-4 text-primary" /></div>
                  <div><p className="text-xs text-muted-foreground">Address</p><p className="font-medium text-sm">{karigar.address || "-"}</p></div>
                </div>
              </div>
            </div>
          </CardContent>
